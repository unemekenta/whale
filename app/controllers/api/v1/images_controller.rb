    ...
    class ImagesController < ApplicationController
      ...
      def create
        ...
        if image.save
          ...
          return_data(STATUS_FAILURE, image.errors.full_messages, '')
        end
      end
      ...
      def destroy
        if @image.user_id == @current_api_v1_user.id
          @image.destroy
          return_data(STATUS_SUCCESS, 'Deleted the image', @image)
        else
          return_data(STATUS_FAILURE, 'You are not authorized to delete this image', '')
        end
      end
